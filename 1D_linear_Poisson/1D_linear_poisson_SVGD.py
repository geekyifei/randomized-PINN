...
parser = argparse.ArgumentParser(description="1D Linear Poisson with Stein Variational Gradient Descent")
parser.add_argument(
    "--rand_seed",
    type=int,
    default=42,
...
nIter = args.nIter
num_print = 20
#path_f   = f'1D_linear_poisson_SVGD_Nres_{Nres}_sigma_{sigma}_test'
path_f   = f'1D_linear_poisson_SVGD_Nres_{Nres}_sigma_{sigma}_nIter_{nIter}_lr_1e-3_median_trick'
...
class PINN():
    def __init__(self, key, layers, dataset, lbt, ubt, k, sigma_r, sigma_b, sigma_p):  
        ...
        flat_params, self.unravel = ravel_pytree(self.params)
        self.num_params = flat_params.shape[0]
        self.log_prob_rec = []
        self.u_rl2e_log = []
        self.u_lpp_log = []
        self.u_std_log = []
    ...
    @partial(jit, static_argnums=(0,))
    def u_pred_vector(self, theta):
        params = self.unravel(theta)
        u_pred_vec = vmap(self.u_net, (None, 0))(params, self.X_data[:,0])
        return u_pred_vec

    @partial(jit, static_argnums=(0,))
    def f_pred_vector(self, theta): 
        params = self.unravel(theta)
        f_pred_vec = vmap(self.res_net, (None, 0))(params, self.X_res[:,0])
        return f_pred_vec  
    
    @partial(jit, static_argnums=(0,))
    def target_log_prob(self, theta):
        prior = jnp.sum(-(theta)**2/(2*self.sigma_p**2))
        r_likelihood = jnp.sum(-(self.y_res.ravel() - self.f_pred_vector(theta))**2/(2*self.sigma_r**2))
        u_likelihood = jnp.sum(-(self.y_data.ravel() - self.u_pred_vector(theta))**2/(2*self.sigma_b**2))
    ...
    #     '''
    #     Compute the correlation length for the kernel using median trick
    #     input: parameter batch (Nsample, Nparams)
    #     output: h
    #     '''
    #     #pairwise_dists = -((theta[:, None, :] - theta))  #one way to compute pairwise distance
    #     #another way, but faster in JAX
    #     pairwise_dist = vmap(vmap(lambda x, y: (x - y), in_axes=(None, 0)), in_axes=(0, None))(theta, theta)
    #     pairwise_dist_sq = (pairwise_dist ** 2).sum(axis=-1)
    #     med_sq = jnp.median(pairwise_dist_sq)
    #     h_sq = med_sq / jnp.log(theta.shape[0] + 1)
    #     return jnp.sqrt(h_sq/2)
    ...
    def svgd_train(self, key, Nsamples, nIter, num_print, bandwidth, u_ref, f_ref):
        ...
        #lr = optimizers.exponential_decay(1e-3, decay_steps=20, decay_rate=0.9)
        #lr = optimizers.exponential_decay(1e-4, decay_steps=1000, decay_rate=0.9)
        self.opt_init, \
        self.opt_update, \
        self.get_params = optimizers.adam(1e-3)
        # self.get_params =   optimizers.adagrad(lr, momentum=0.9)
        ...
        for it in pbar:
            ...
            if it % num_print == 0:
                ...
                log_prob = vmap(self.target_log_prob)(theta)
                ...
                self.log_prob_rec.append(jnp.mean(log_prob))
                self.u_rl2e_log.append(rl2e_u)
                self.u_lpp_log.append(lpp_u)
                self.u_std_log.append(jnp.mean(u_test_std))
                self.f_rl2e_log.append(rl2e_f)
                self.f_lpp_log.append(lpp_f)
                self.f_std_log.append(jnp.mean(f_test_std))
                
                pbar.set_postfix({'Log prob': jnp.mean(log_prob), 
                          'correlation': h,
                          'u_lpp':lpp_u,
                          'f_lpp': lpp_f})
...
model = PINN(key1, layers_u, dataset, lbt, ubt, k, sigma_r, sigma_b, sigma_p)
ts = perf_counter()
samples = model.svgd_train(key2, Nsamples, nIter = nIter, num_print = num_print, bandwidth = -1, u_ref = u, f_ref = f)
timings = perf_counter() - ts
print(f"SVGD: {timings} s")
print(f"SVGD: {timings} s", file = f_rec)
np.savetxt(os.path.join(path_f,f'SVGD_posterior_samples_Nres_{Nres}_sigma_{sigma}_Nsamples_{Nsamples}_nIter_{nIter}.out'), samples)
...
fig.tight_layout()
plt.savefig(os.path.join(path_fig,f'1D_linear_poisson_SVGD_Nres_{Nres}_sigma_{sigma}_Nsamples_{Nsamples}_nIter_{nIter}_upred.png'))
plt.show()
...
fig.tight_layout()
plt.savefig(os.path.join(path_fig,f'1D_linear_poisson_SVGD_Nres_{Nres}_sigma_{sigma}_Nsamples_{Nsamples}_nIter_{nIter}_fpred.png'))
plt.show()
...
ax = fig.add_subplot()
ax.plot(t, -np.array(model.log_prob_rec), color='blue', label='Negative log prob')
ax.set_yscale('log')
ax.set_ylabel('Loss',  fontsize = 16)
ax.set_xlabel('Epochs', fontsize = 16)
ax.legend(loc='upper right', fontsize = 14)
fig.tight_layout()
plt.savefig(os.path.join(path_fig,f'1D_linear_poisson_SVGD_loss.png'))
plt.show()
...
fig = plt.figure(constrained_layout=False, figsize=(4, 4), dpi = 300)
ax = fig.add_subplot()
ax.plot(t, np.array(model.u_lpp_log), label='u')
ax.plot(t, np.array(model.f_lpp_log), label='f')
ax.set_ylabel('LPP',  fontsize = 16)
ax.set_xlabel('Epochs', fontsize = 16)
ax.legend(loc='upper right', fontsize = 14)
fig.tight_layout()
fig.savefig(os.path.join(path_fig,'test_lpp.png'))

t = np.arange(0, nIter, num_print)
fig = plt.figure(constrained_layout=False, figsize=(4, 4), dpi = 300)
ax = fig.add_subplot()
ax.plot(t, np.array(model.u_std_log), label='u')
ax.plot(t, np.array(model.f_std_log), label='f')
ax.set_ylabel('std',  fontsize = 16)
ax.set_xlabel('Epochs', fontsize = 16)
ax.set_yscale('log')
ax.legend(loc='upper right', fontsize = 14)
fig.tight_layout()
fig.savefig(os.path.join(path_fig,'test_std.png'))
